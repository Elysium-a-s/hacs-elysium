...
class ElysiumSelect(ElysiumHelperMixin, SelectEntity):
    ...
    def write_record_state(self):
        self._attr_options=list(self._record.get("config",{}).get("options",["Option 1"]))
        self._attr_current_option=str(self._record.get("state") or self._attr_options[0])
        self._attr_icon="mdi:form-dropdown"
    async def async_select_option(self,option):
        if option not in self._attr_options: raise ValueError("Unknown option")
        self._record["state"]=option;self.write_record_state();await self.persist();self.async_write_ha_state()
